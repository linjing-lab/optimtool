...
&emsp;&emsp;optimtool采用了北京大学出版的《最优化：建模、算法与理论》这本书中的部分理论方法框架，运用了`Numpy`包高效处理数组间运算等的特性，巧妙地应用了`Sympy`内部支持的jacobian等方法，并结合Python内置函数dict与zip实现了Sympy矩阵到Numpy矩阵的转换，最终设计的用在最优化科学研究的Python工具包。
