...
&emsp;&emsp;optimtool adopts theoretical frameworks in book <Optimization: Modeling, Algorithms and Theory> published by Peking University Press, `Numpy` is used to handle operations between numerical arrays efficiently. Applied jacobian and methods of `Sympy` to support symbolic differentiation, and realized fast computation of Numpy matrix combined with sp.lambdify. The optimization toolbox for scientific research is designed for Researchers and Developers.
