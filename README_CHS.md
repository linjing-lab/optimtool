...
&emsp;&emsp;因为在求解不同的目标函数的全局或局部收敛点时，会有不同的求取收敛点的方法，而且在研究过程中不同领域的研究方法不断被提出修改、完善、扩充，所以这些方法成了现在人们口中的`最优化方法`。 此项目中的所有内部支持的算法，都是在范数、导数、凸集、凸函数、共轭函数、次梯度和最优化理论等基础方法论的基础上进行设计与完善的。
